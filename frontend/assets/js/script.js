...
function AddPlayer(e){
      ...
      if(checkfiledTextValidation(inputs) == 0){
        ...
        let rating = inputs.shooting + inputs.dribbling + inputs.passing / 3 
        localStorage.setItem("myarray",JSON.stringify(PlayerList)); 
...
function checkfiledTextValidation(inputs){
  ...
  let foundProblems = 0 ;
  if(inputs.name ==''||!regexname.test(inputs.name) ) (foundProblems = 1 , errortext.textContent = "Name requiredalid or invalid") ;
